...
def main():
    ...
    n = nicoExtractor(username, password, mysql_username, mysql_password, db_name)
...
class nicoExtractor():
    ...
    def __init__(self, username, password, mysql_username, mysql_password, db_name):
        self.lower_bound = "2007-1-1"
        self.err_log_loc = "../Videos/error_logs/"
        self.tmp_video_loc = "../Videos/new_tmp/"
        self.cookie_file = "./cookies.txt"
        #self.video_loc = "../Videos/"
    ...
    def setNewCookie(self):
        ...
        os.system("youtube-dl -u '%s' -p '%s' --cookies %s www.nicovideo.jp/watch/" % (self.username, self.password, self.cookie_file))
    ...
    #
    #need to add base case for recursive call
    #
    def fillChanStack(self, uid, pageNumber = 1):
        resp = urllib.request.urlopen("http://www.nicochart.jp/user/" + str(uid) + "?page=" + str(pageNumber))
        ...
        for link in soup.find_all('a', href=re.compile('^watch/')):
            ...
            curs = conn.cursor()
            try:
                curs.execute("INSERT INTO auth_dl_stack (video_ID, author_id) VALUES (%s, %s, %s)", (video_id, uid))
                conn.commit()
        ...
        self.fillChanStack(uid, pageNumber + 1)

    
            

    def dlChanStack(self, uid):
        query = "select video_ID from auth_dl_stack where video_ID not in (select video_ID from nicovideo where downloaded = 1) AND video_ID not in (select video_ID from delVideos) AND author_id = %s order by Upload_Date desc"
        conn = self.getNewConn()
        curs = conn.cursor()
    ...
    def vidDl(self, video_id, method, overwrites = False):
        ...
        except (ConnectionError, socket.gaierror, MaxRetryError, AssertionError, NewConnectionError) as e:
            print(str(type(e).__name__), "on %s" % video_id)
            time.sleep(20)
            self.vidDl(video_id, method, overwrites)
    ...
    def getNewConn(self, **kwargs):
        return pymysql.connect(user=self.mysql_username, password = self.mysql_password, database=self.db_name, charset='utf8', **kwargs)
    ...
    def dlChannel(self, uid, pageNum = 1):
        resp = urllib.request.urlopen("http://www.nicochart.jp/user/" + str(uid) + "?page=" + str(i))
        #message displayed when page is invalid
        if("に一致する動画は見つかりませんでした" in resp.content ):
            return
        soup = BeautifulSoup(resp, from_encoding=resp.info().get_param('charset'))
        for link in soup.find_all('a', href=re.compile('^watch/')):
    ...
    def copyDriver(self, path, video_ID):
        for f in glob.glob(str(path) + str(video_ID) + ".*"):
            ...
            if(fileExtension == "part"):
                ...
                self.copyLinearProbe(currentFilePath, file, "../Videos")
